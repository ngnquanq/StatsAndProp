\subsection{1. Khái niệm và cơ sở lý thuyết của tự tương
quan}\label{khuxe1i-niux1ec7m-vuxe0-cux1a1-sux1edf-luxfd-thuyux1ebft-cux1ee7a-tux1ef1-tux1b0ux1a1ng-quan}

\subsubsection{1.1. Mô hình hồi quy tuyến tính bội và sai số ngẫu
...
\subsubsection{1.2. Giả định không tự tương
...
\subsubsection{1.3. Khái niệm tự tương quan}
...
\subsubsection{1.4. Sai số và thặng dư sau ước lượng}
...
Vector thặng dư $e$ được sử dụng như một đại diện quan sát được gần đúng cho vector sai số ngẫu nhiên \(\boldsymbol{\varepsilon}\). Vì vậy, trong thực hành, việc kiểm tra tự tương quan thường được thực hiện dựa trên các thặng dư thu được từ mô hình hồi quy đã ước lượng. 

\subsection{2. Nguyên nhân dẫn đến tự tương quan}
...
\subsubsection{2.1. Nguyên nhân do cấu trúc phụ thuộc của dữ liệu}
...
\subsubsection{2.2. Nguyên nhân do mô hình chưa được đặc tả phù hợp}
...
\subsection{3. Hậu quả thống kê của tự tương
...
\subsubsection{3.1. Tự tương quan làm sai cấu trúc phương sai của vector
sai
...
Ma trận phương sai - hiệp phương sai của vector sai số có dạng tổng quát là:
...
là phương sai của sai số tại quan trắc \(i\), còn:
...
là hiệp phương sai giữa sai số tại hai quan trắc khác nhau.

Trong mô hình hồi quy tuyến tính cổ điển, ta giả định các sai số có cùng phương sai:
...
Đồng thời, ta cũng giả định sai số giữa các quan trắc khác nhau không tương quan với nhau:
...
Khi hai giả định này cùng đúng, ma trận phương sai - hiệp phương sai của vector sai số trở thành:
...
Công thức này có hai ý nghĩa. Thứ nhất, các phần tử trên đường chéo chính đều bằng \(\sigma^2\), nghĩa là các sai số có cùng phương sai. Thứ hai, các phần tử ngoài đường chéo đều bằng 0, nghĩa là sai số giữa các quan trắc khác nhau không tương quan với nhau.
...
Tuy nhiên, khi có tự tương quan, các sai số trong vector \(\boldsymbol{\varepsilon}\) có quan hệ với nhau. Nói cách khác, sai số ở quan trắc này có thể tương quan với sai số ở quan trắc khác.

Ví dụ, trong dữ liệu chuỗi thời gian, sai số tại thời điểm \(t\) có thể tương quan với sai số tại thời điểm \(t-1\):

\[
\operatorname{Cov}(\varepsilon_t,\varepsilon_{t-1}) \neq 0
\]

Tổng quát hơn, khi có tự tương quan:

\[
\operatorname{Cov}(\varepsilon_i,\varepsilon_j) \neq 0
\quad \text{với một số } i \neq j
\]

Điều này làm cho các phần tử ngoài đường chéo của ma trận phương sai - hiệp phương sai không còn bằng 0. Khi đó, ma trận phương sai - hiệp phương sai của vector sai số không còn là:
...
mà phải được viết tổng quát hơn là:
...
Trong đó, \(\Omega\) là ma trận chứa toàn bộ phương sai và hiệp phương sai của các sai số. Các phần tử trên đường chéo chính của \(\Omega\) là phương sai của từng sai số, còn các phần tử ngoài đường chéo là hiệp phương sai giữa các sai số khác nhau.
...
Nói cách khác, \(\sigma^2 I\) chỉ là một trường hợp đặc biệt của \(\Omega\). Trường hợp này chỉ đúng khi sai số có phương sai không đổi và không có tự tương quan. Khi có tự tương quan, các phần tử ngoài đường chéo khác 0, nên ta không thể dùng \(\sigma^2 I\) để mô tả cấu trúc sai số nữa.
...
thì sai số chuẩn của các hệ số hồi quy có thể bị tính sai.

\subsubsection{3.2. Sai số chuẩn sai làm kiểm định t và p-value không đáng tin cậy}
...
\subsubsection{3.3. Ước lượng phương sai sai số và hệ số $R^2$ cần được diễn giải thận trọng}
...
\subsubsection{3.4. Kiểm định F, khoảng tin cậy và khoảng dự báo bị ảnh hưởng}
...
\subsection{4. Phát hiện tự tương quan}
...
\subsubsection{4.1. Phương pháp đồ thị}
...
\paragraph{4.1.1. Vẽ thặng dư theo thời gian}
...
\includegraphics[width=0.8\textwidth]{media/plot_ts.png}

\paragraph{4.1.2. Vẽ thặng dư theo vị trí hoặc khu vực}
...
Tuy nhiên, phương pháp đồ thị chỉ giúp nhận diện sơ bộ mẫu hình của thặng dư và có thể mang tính chủ quan. Vì vậy, kết luận chính thức cần dựa thêm trên các kiểm định thống kê phù hợp với loại dữ liệu.


\subsubsection{4.2. Kiểm định
...
\subsubsection{Cách đọc kết quả bằng bảng
...
\subsubsection{Hạn chế của kiểm định
...
\subsubsection{4.4. Kiểm định tự tương quan trên R}
...
\includegraphics[width=0.8\textwidth]{media/summary.png}
...
\paragraph{Kiểm định Durbin--Watson}
...
\includegraphics[width=0.8\textwidth]{media/d.png}
...
\paragraph{Kiểm định Breusch--Godfrey}
...
\includegraphics[width=0.8\textwidth]{media/bg_1.png}
...
\includegraphics[width=0.8\textwidth]{media/bg_2.png}
