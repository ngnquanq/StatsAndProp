...
\begin{frame}{The Mathematics (Brief)}
    ...
    \small{\textcolor{customred}{Key insight:} As $h$ increases, 
    more points contribute to the density estimate, 
    which may cause performance issues.
    Also if we set $h$ too large, we may suffer from over-smoothing.}
\end{frame}
...
\begin{frame}{From KDE to Mean Shift}
    ...
    \small{\textcolor{customred}{Key insight:} The function $g = -k'$ ensures we \textbf{move along} the gradient direction (toward increasing density). This weighted mean tells us where the mode is!}
\end{frame}
...
\begin{frame}{Common Kernel Profiles}
    ...
    \begin{columns}[T]
        ...
        \begin{column}{0.5\textwidth}
            ...
            \small{\textcolor{customred}{Why Epanechnikov? and not Flat?}}
            \begin{itemize}
                ...
                \item Flat kernel is not used because $g(s) = 0$ everywhere $\Rightarrow$ no gradient info!
            \end{itemize}
        \end{column}
    \end{columns}
\end{frame}
